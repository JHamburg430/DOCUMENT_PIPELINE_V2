...
def test_validate_answer_restores_user_supplied_model_scope_when_chunk_inherits_it():
    result = SearchResult(
        chunk_id="transfer",
        score=0.9,
        title="Transfer time",
        ...
        pages=[12],
        section_path=["Transfer time"],
        content="Image transfer in 5 megapixel mode takes 29.2 ms or 11.7 ms.",
...
def test_validate_answer_does_not_prefix_one_side_of_multi_model_query():
    result = SearchResult(
        ...
        content="The transfer times are 29.2 ms and 11.7 ms.",
        metadata={},
    )
    generated = AnswerResponse(
        answer="The transfer times are 29.2 ms and 11.7 ms.",
        confidence="high",
        used_documents=[],
        citations=[
            {
    ...
    validated = validate_answer(
        generated,
        [result],
        query="Compare image transfer for VJ-H500CX and LJ-S8000.",
    )

    assert not validated.answer.startswith("For VJ-H500CX,")
    assert not validated.answer.startswith("For LJ-S8000,")
...
def test_generate_answer_extracts_verified_single_hop_evidence_without_model(monkeypatch):
    result = SearchResult(
        chunk_id="verified-parent",
        score=1.0,
        title="W500 Manual",
        ...
        content=(
            ...
            "(default). To change this value, open the calibration menu."
        ),
        metadata={
            "chunk_type": "parent_section",
            "agent_context_reasons": ["required_claim:lookup"],
    ...
    assert answer.citations[0]["chunk_id"] == "verified-parent"
    assert trace["final_answer"]["answer_source"] == "deterministic_verified_evidence"
    assert trace["final_answer"]["model"] is None
