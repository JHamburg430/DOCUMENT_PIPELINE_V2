...
def analyze_query(query: str) -> QueryAnalysis:
    ...
    protocol_terms = sorted(
        {
            ...
            for match in re.findall(
                r"\b(?:ethernet/?ip|ethercat|profinet|modbus|tcp/?ip|udp|rs-232c?|rs-485|usb|io-link|canopen|cc-link)\b",
                query,
                flags=re.IGNORECASE,
            )
        }
    ...
    if any(word in lowered for word in ["spec", "datasheet", "voltage", "current", "dimension", "value", "parameter"]):
    ...
    if protocol_terms:
        preferred_metadata_filters["protocol_terms"] = protocol_terms
    if not types:
